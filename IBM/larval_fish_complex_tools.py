...
class PelagicPlanktonDrift(OpenDrift3DSimulation):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._add_config('biology:constant_ingestion', 'float(min=0.0, max=1.0, default=0.5)',
                         comment='Ingestion constant')
        self._add_config('biology:active_metab_on', 'float(min=0.0, max=1.0, default=1.0)', comment='Active metabolism')
        self._add_config('biology:attenuation_coefficient', 'float(min=0.0, max=1.0, default=0.18)',
                         comment='Attenuation coefficient')
        self._add_config('biology:fraction_of_timestep_swimming', 'float(min=0.0, max=1.0, default=0.15)',
                         comment='Fraction of timestep swimming')
        self._add_config('biology:lower_stomach_lim', 'float(min=0.0, max=1.0, default=0.3)',
    ...
    def updateEggDevelopment(self):
        ...
        amb_duration = np.exp(
            3.65 - 0.145 * self.environment.sea_water_temperature)  # Total egg development time (days) according to ambient temperature (Ellertsen et al. 1988)
        days_in_timestep = self.time_step.total_seconds() / (
                    60 * 60 * 24)  # The fraction of a day completed in one time step
        amb_fraction = days_in_timestep / (
            amb_duration)  # Fraction of development time completed during present time step
        ...
        self.elements.hatched[
            self.elements.stage_fraction >= 1] = 1  # Eggs with total development time completed are hatched (1)
    ...
    def updateVertialPosition(self, length, oldLight, currentLight, currentDepth, stomach_fullness, dt):
        ...
        if (
                oldLight <= currentLight and stomach_fullness >= lowerStomachLim):  # If light increases and stomach is sufficiently full, go down
    ...
    def updatePlanktonDevelopment(self):
        ...
        for ind in range(len(self.elements.lat)):
            if (self.elements.hatched[ind] >= 1):
                ...
                larvamm, larvawgt, stomach, ingrate, growthrate, sfullness = bioenergetics.bioenergetics.growth(
                    self.elements.length[ind],
