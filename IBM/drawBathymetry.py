import IBM.laplacefilter as laplacefilter
import IBM.mpl_util as mpl_util
...
def addBathymetryROMS(confobj):
    ...
    confobj.mymap.contourf(bathX, bathY, -bathZ,
                           cmap=mpl_util.LevelColormap(levels,
                                                       cmap=cm.Greys_r),
                                                       alpha=0.5)
    print("Adding bathymetry from ROMS min,mean,max depths: {},{},{}".format(np.min(bathZ), np.mean(bathZ),
                                                                             np.max(bathZ)))
