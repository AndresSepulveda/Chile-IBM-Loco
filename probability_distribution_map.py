...
import IBM.drawBathymetry as drawBathymetry
import IBM.drawCircleOfDistance as drawCircleOfDistance
...
def createInputFilename(confobj):
    ...
    confobj.outputFilename = 'results/{}_opendrift_{}_to_{}_experiment_{}.nc'.format(
        confobj.species,
        startDate, endDate,
        confobj.experiment)
...
def get_pos(confobj):
    ...
    for i, index in enumerate(indexes):
        if index is not None:
            ...
            lons.append(d.lon[i, :].values)
            lats.append(d.lat[i, :].values)
            z.append(d.z[i, :].values)
...
def createBins(confobj):
    ...
    print('=> created binned array of domain of grid cell size (%s,%s) with resolution %s' % (
        confobj.deltaX, confobj.deltaY, confobj.requiredResolution))
...
def make_map(confobj):
    ...
    createBins(confobj)
    confobj.mymap.drawparallels(confobj.lat_bins, linewidth=0.2, fmt='%g' + 'E', fontsize=5, color='gray',
                                labels=[True, False, False, False])
    confobj.mymap.drawmeridians(confobj.lon_bins, linewidth=0.2, fmt='%g' + 'E', fontsize=5, color='gray',
    ...
    if confobj.plot_type == 'heatmap':
        ...
        print(nlevels, lats, lons)
        Xd, Yd, density = get_density(lats, lons, nlevels, confobj)

        cs = confobj.mymap.pcolormesh(Xd, Yd, density, cmap=confobj.cmap, edgecolors='face',
                                      linewidths=0.1)  # ,alpha=0.8)  #norm=norm,
        ...
        X, Y = drawCircleOfDistance.createCircleAroundWithRadius(confobj.st_lats[0], confobj.st_lons[0],
                                                                 confobj.releaseRadius / 1000.)
