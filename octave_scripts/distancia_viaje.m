...
for jj=1:length(a_lat)
   ...
   bb=sum(normalized_particulas_od(jj,:));  % QC - Must be 100
   auxqcq=[jj];
   for kk=length(a_lat):-1:length(a_lat)-9  % Increased  to 10, as IFOP requested
...
for jj=1:length(a_lat)
   ...
   aux_norm=normalized_particulas_od(:,jj);
   sort_norm=sort(aux_norm);
%%   bb=sum(normalized_particulas_od(jj,:));  Nonsense here.
   auxdnd=[jj];
   for kk=length(a_lat):-1:length(a_lat)-9  % Increased  to 10, as IFOP requested
      indx=find(aux_norm == sort_norm(kk));
      auxdnd=[auxdnd, indx(1), sort_norm(kk)];
   end
   dedonde=[dedonde; auxdnd];
end
...
print('-dpng',[prefix,'_distancias.png'])
...
filename=['diez_conexiones_particulas.txt'];
fid = fopen(filename,'w+');
...
filename=['diez_origenes_particulas.txt'];
fid = fopen(filename,'w+');
for i=1:size(dedonde,1)
   fprintf(fid,'%i %i %.1f %i %.1f %i %.1f %i %.1f %i %.1f %i %.1f %i %.1f %i %.1f %i %.1f %i %.1f',dedonde(i,:));
   fprintf(fid,'\n');
end
fclose(fid);
